package com.danielcorroto.directorius.view;

/**
 * Constantes para ubicar las rutas de recursos
 * 
 * @author Daniel Corroto Quirós
 *
 */
public class ResourcePath {
	/**
	 * CSS de Bootstrap
	 */
	public static final String CSS_BOOTSTRAP = "/web/css/bootstrap.min.css";
	/**
	 * CSS de estilos del contacto
	 */
	public static final String CSS_VCARD = "/web/css/vcard.css";
	/**
	 * HTML base para mostrar información del contacto
	 */
	public static final String HTML = "/web/vcard.html";
	/**
	 * HTML base para mostrar información de un grupo de direcciones del contacto
	 */
	public static final String HTML_GROUP_ADDRESS = "/web/vcard_group_address.html";
	/**
	 * HTML base para mostrar información de un grupo de categorias del contacto
	 */
	public static final String HTML_GROUP_CATEGORY = "/web/vcard_group_category.html";
	/**
	 * HTML base para mostrar información de un grupo de correos del contacto
	 */
	public static final String HTML_GROUP_EMAIL = "/web/vcard_group_email.html";
	/**
	 * HTML base para mostrar información de un grupo de teléfonos del contacto
	 */
	public static final String HTML_GROUP_PHONE = "/web/vcard_group_phone.html";
	/**
	 * HTML base para mostrar información de una dirección del contacto
	 */
	public static final String HTML_SECTION_ADDRESS = "/web/vcard_section_address.html";
	/**
	 * HTML base para mostrar información de una categoria del contacto
	 */
	public static final String HTML_SECTION_CATEGORY = "/web/vcard_section_category.html";
	/**
	 * HTML base para mostrar información de un correo del contacto
	 */
	public static final String HTML_SECTION_EMAIL = "/web/vcard_section_email.html";
	/**
	 * HTML base para mostrar información de un teléfono del contacto
	 */
	public static final String HTML_SECTION_PHONE = "/web/vcard_section_phone.html";
	/**
	 * HTML base para mostrar la foto por defecto
	 */
	public static final String HTML_SECTION_PHOTO_DEFAULT = "/web/vcard_section_photo_default.html";
	/**
	 * Imagen del logo
	 */
	public static final String IMG_LOGO = "/img/logo.png";
	/**
	 * Imagen de menú File / New
	 */
	public static final String IMG_MENU_FILE_NEW = "/img/menu_file_new.png";
	/**
	 * Imagen de menú File / Open
	 */
	public static final String IMG_MENU_FILE_OPEN = "/img/menu_file_open.png";
	/**
	 * Imagen de menú File / Exit
	 */
	public static final String IMG_MENU_FILE_EXIT = "/img/menu_file_exit.png";
	/**
	 * Imagen de menú Contact / Add
	 */
	public static final String IMG_MENU_CONTACT_ADD = "/img/menu_contact_add.png";
	/**
	 * Imagen de menú Contact / Edit
	 */
	public static final String IMG_MENU_CONTACT_EDIT = "/img/menu_contact_edit.png";
	/**
	 * Imagen de menú Contact / Remove
	 */
	public static final String IMG_MENU_CONTACT_REMOVE = "/img/menu_contact_remove.png";
	/**
	 * Imagen de menú Contact / Statistics
	 */
	public static final String IMG_MENU_CONTACT_STATISTICS = "/img/menu_contact_statistics.png";
	/**
	 * Imagen de menú Birthday / Today
	 */
	public static final String IMG_MENU_BIRTHDAY_TODAY = "/img/menu_birthday_today.png";
	/**
	 * Imagen de menú Birthday / Within a week
	 */
	public static final String IMG_MENU_BIRTHDAY_WITHINWEEK = "/img/menu_birthday_withinweek.png";
	/**
	 * Imagen de menú Birthday / Within a month
	 */
	public static final String IMG_MENU_BIRTHDAY_WITHINMONTH = "/img/menu_birthday_withinmonth.png";
	/**
	 * Imagen de menú Birthday / This week
	 */
	public static final String IMG_MENU_BIRTHDAY_THISWEEK = "/img/menu_birthday_thisweek.png";
	/**
	 * Imagen de menú Birthday / This month
	 */
	public static final String IMG_MENU_BIRTHDAY_THISMONTH = "/img/menu_birthday_thismonth.png";
	/**
	 * Imagen de menú Help / About
	 */
	public static final String IMG_MENU_HELP_ABOUT = "/img/menu_help_about.png";
	/**
	 * Imagen de editar contacto / Añadir categoría
	 */
	public static final String IMG_EDIT_CONTACT_CATEGORY_ADD = "/img/edit_contact_category_add.png";
	/**
	 * Imagen de editar contacto / Añadir teléfono
	 */
	public static final String IMG_EDIT_CONTACT_PHONE_ADD = "/img/edit_contact_phone_add.png";
	/**
	 * Imagen de editar contacto / Añadir email
	 */
	public static final String IMG_EDIT_CONTACT_EMAIL_ADD = "/img/edit_contact_email_add.png";
	/**
	 * Imagen de editar contacto / Añadir dirección
	 */
	public static final String IMG_EDIT_CONTACT_ADDRESS_ADD = "/img/edit_contact_address_add.png";
	/**
	 * Imagen de editar contacto / Editar
	 */
	public static final String IMG_EDIT_CONTACT_EDIT = "/img/edit_contact_edit.png";
	/**
	 * Imagen de editar contacto / Eliminar
	 */
	public static final String IMG_EDIT_CONTACT_REMOVE = "/img/edit_contact_remove.png";
	/**
	 * Imagen de subir elemento
	 */
	public static final String IMG_EDIT_CONTACT_ARROW_UP = "/img/edit_contact_arrow_up.png";
	/**
	 * Imagen de bajar elemento
	 */
	public static final String IMG_EDIT_CONTACT_ARROW_DOWN = "/img/edit_contact_arrow_down.png";
}
